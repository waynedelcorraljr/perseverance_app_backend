class CheckForNewJob < ApplicationJob
  ...
  def perform(*args)
    ...
    last_photo_earthdate = Photo.last.earth_date.to_datetime

    if total_days_on_api && total_days_in_db < total_days_on_api
      Earthdate.create_earthdates_from_api(earthdates)
      starting_index = total_days_on_api - (total_days_on_api - total_days_in_db)
      new_photos = Photo.call_nasa_for_photos(starting_index)
      Photo.create_photos_from_api(new_photos)
